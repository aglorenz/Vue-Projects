// ramdon integer helper function
...
Vue.component("letter-button", {
    ...
    template: "<button class='keboard-row-letter' :id='letter' :disabled='disabled' @click='clicked()'>{{ letter }}</button>",
    data: function() {
        return {
            disabled: false
        };
...
var app = new Vue({
    ...
    methods: {
        ...
        // draws thew appropriate part of the hanging man and/or 'game over'
        updateCanvas: function(ctx) {
            ...
            // draw the left arm and handle game over
                ...
                ctx.fillText("Game Over", this.canvas.width * 0.4 - 60, this.canvas.height * 0.9);
                this.gameOver = true;
                this.lose = true;
        ...
        // check the chose letter when a letter rcomponent emits 'check'
        check: function(letter) {
            if (!this.gameOver) {
                ...
                if (!this.wordDivs.some(function(value) {return value == ""})) {
                    ...
                    this.ctx.fillText("You Win", this.canvas.width * 0.4 - 60, this.canvas.height * 0.9);
                }
        ...
        //reset the game to on-player mode and chooses a new word
        onePlayer: function() {
            if(this.twoPlayers) {
        ...
        twoPlayer: function() {
            if (!this.twoPlayers) {
                ...
                while (!letters.test(this.currentWord)) {
                    try {
                        this.curentWord = prompt("Only letters please! Enter a word:").toUpperCase();
                    }
                    catch(e) {
                        this.onePlayer();
                        return;
